...
class Pay extends MY_Controller {
    ...
    public function stripe()
    {
        ...
        if (($amount_dollars < $min_payment) || ($amount_dollars > $max_payment)) {
            $message = sprintf(__('Ошибка! Деньги с вашего счета не списаны! Сумма платежа должна быть не менее %s и не более %s USD'), $min_payment, $max_payment);
        ...
        $message = sprintf(__('Баланс успешно пополнен на сумму %s USD'), round(($charge->amount / 100), 2));
        $this->session->set_flashdata('payment_message', $message);
        redirect('/advertiser/payments');
    }
